...
- [~] Application `GPS citoyen` (signalement géolocalisé et lieux utiles) — premier écran fonctionnel dans `app/gps-citoyen` (Compose) : signale une poubelle qui débordé avec position + note optionnelle, stocke l'objet `waste_report` (mode invité) et affiche les signalements connus de l'appareil ; APK debug/release compilent (`./gradlew assembleDebug`/`assembleRelease`). Manque encore : identité durable (onboarding clé racine/appareil), photo, catégories autres que `overflowing_bin`, et bien sûr toute synchronisation réelle. Localisation via `android.location.LocationManager` du framework, jamais les Play Services (Fused Location) — l'appareil cible (e/OS) ne les embarque pas.
...
- [x] `core/storage-android` : implémentation Android (`android.database.sqlite`) de `SignedObjectStore` et `GuestQuota` — remplacement mécanique des implémentations JDBC desktop de `core/storage`, comme annoncé dans leur documentation ; SDK Android installé dans cet environnement (`/opt/android-sdk`, platform 34, build-tools 34.0.0) une fois `dl.google.com` autorisé côté politique réseau
