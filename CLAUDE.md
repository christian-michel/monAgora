...
- [x] Objets d'identité et de révocation (`identity_declaration`, `device_authorization`, `device_revocation`, `identity_revocation`) — `core/objects/identity` (`IdentityObjects` : création + lecture typée des 4 payloads, conforme à `docs/identite-revocation.md` section 3) ; **la résolution de confiance de la section 4** (un objet signé par l'appareil D est-il actuellement valide pour l'identité R, en tenant compte des `device_revocation`/`identity_revocation` connus) **n'est pas encore implémentée** — étape distincte, à faire
