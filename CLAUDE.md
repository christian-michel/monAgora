...
- [x] Synchronisation locale à deux appareils (Bluetooth) — `core/sync` couvre la couche protocole (messages, curseur, validation+stockage+comptage), testée en JVM pur avec de vrais flux (`SyncSession`, testée via `PipedInputStream`/`PipedOutputStream`, sans matériel) ; `core/sync-android` branche ça sur de vrais sockets Bluetooth RFCOMM (`BluetoothSyncServer`/`BluetoothSyncClient`, appareils déjà appairés seulement — pas de découverte active) ; câblé dans `app/gps-citoyen` (bouton "Synchroniser avec ...", serveur démarré au lancement). **Compile et tourne en JVM pur, mais jamais encore testé entre deux téléphones réels** — Wi-Fi Direct pas fait (Bluetooth choisi en premier, cf. discussion), à faire au besoin (débit plus élevé, utile pour des fichiers)
- [ ] Test de bout en bout hors-ligne (deux téléphones, aucun serveur) — **prochaine étape concrète** : installer l'APK sur les deux POCO F1, appairer en Bluetooth, vérifier qu'un signalement créé sur l'un apparaît sur l'autre après synchronisation
- [~] Application `GPS citoyen` (signalement géolocalisé et lieux utiles) — écran fonctionnel dans `app/gps-citoyen` (Compose) : signale une poubelle qui débordé avec position + note optionnelle, stocke l'objet `waste_report` (mode invité), affiche les signalements connus de l'appareil, et synchronise via Bluetooth avec un appareil appairé ; APK debug/release compilent (`./gradlew assembleDebug`/`assembleRelease`). Manque encore : identité durable (onboarding clé racine/appareil — la clé de session invité sert aussi de `device_pubkey` de synchro pour l'instant, voir la note dans `MainActivity.kt`), photo, catégories autres que `overflowing_bin`. Localisation via `android.location.LocationManager` du framework, jamais les Play Services (Fused Location) — l'appareil cible (e/OS) ne les embarque pas.
- [x] `core/objects/wastereport` : payload `waste_report` v1 (docs/format-objets-signes.md, section 5) — lat/lon/category obligatoires, note/photo_hash optionnels ; `createAsGuest` pour l'écriture en mode invité (autorisée pour cette application par la constitution, section 8)
- [x] `core/storage-android` : implémentation Android (`android.database.sqlite`) de `SignedObjectStore` et `GuestQuota` — remplacement mécanique des implémentations JDBC desktop de `core/storage`, comme annoncé dans leur documentation
...
- [x] Résolution de confiance (`docs/identite-revocation.md` section 4) — `core/trust` (`TrustResolver.resolve(root, device)` : les 3 règles — device_authorization valide, pas de device_revocation, pas d'identity_revocation valide — chaque objet candidat re-vérifié en signature, pas seulement lu depuis le stockage)
- [x] Mode invité : génération/destruction de clé éphémère, limite anti-abus locale (cf. `docs/constitution-technique.md` section 8) — `GuestSession` (`core/identity` : clé jetable, clé privée jamais exposée, détruite à `close()`), `SignedObjects.createAsGuest` (`core/objects`), `TrustResolver.isRegisteredDevice`/`isFromGuestIdentity` (`core/trust` : reconnaissance par absence de `device_authorization`), `GuestQuota`/`SqliteGuestQuota` (`core/storage` : compteur par appareil et par jour, la valeur numérique de la limite reste un choix applicatif non fixé ici) ; **portée par défaut lecture/écriture par application (tableau de la section 8) pas encore appliquée** — n'a de sens qu'une fois une application réelle construite
